...
- `docs/design/jila_canonical_design_principles.md` (UX-P-001..)
- `docs/design/jila_design_decision_register.md` (DECIDED items, components/patterns)
...
**Contrast text (onColor)**
- `onPrimary`: `#000000` (dark text on teal for AAA contrast — 7.5:1)
...
- Web token docs may define:
  ...
  - glass effect implementation details (CSS-specific)

---

## 5) Effects (v1)

Intent: standardize visual effects that apply across platforms while allowing platform-specific implementation.

### 5.1 Glass surface effect (v1)

The glass effect provides depth and hierarchy for elevated surfaces (modals, sheets, floating panels).

**Parameters:**
- Background alpha: `0.70` (70% opacity of surface color)
- Blur radius:
  - Panels/cards: `12px`
  - Modals/dialogs: `24px`
...
**Usage guidelines:**
- Use sparingly — glass is for elevated, focused UI (modals, bottom sheets, floating actions)
- Do not use on primary content surfaces (cards in lists, main page backgrounds)
- Ensure text contrast is maintained over blurred backgrounds (UX-P-001/009)
- Platform implementations may adjust blur intensity for performance, but must maintain the layered/elevated feel

**Platform implementation notes:**
- Web: CSS `backdrop-filter: blur()` + rgba background
- Mobile (React Native): Platform blur APIs (iOS UIBlurEffect, Android RenderEffect) or fallback to solid alpha
